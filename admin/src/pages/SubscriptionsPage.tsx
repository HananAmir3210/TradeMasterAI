...
export default function SubscriptionsPage() {
  ...
  const { data: subscriptionsData, isLoading } = useQuery({
    ...
    queryFn: () => mockAdminApi.getSubscriptions({
  ...
  const cancelSubscriptionMutation = useMutation({
    mutationFn: ({ subscriptionId, immediate }: { subscriptionId: string; immediate: boolean }) =>
      mockAdminApi.cancelSubscription(subscriptionId, immediate),
    onSuccess: () => {
  ...
  const updateSubscriptionMutation = useMutation({
    mutationFn: ({ subscriptionId, data }: { subscriptionId: string; data: any }) =>
      mockAdminApi.updateSubscription(subscriptionId, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['subscriptions'] })
      toast({
  ...
  const handleExport = async () => {
    try {
      const blob = await mockAdminApi.exportData('subscriptions', 'csv')
    ...
    <div className="space-y-2 animate-fade-in ml-0">
      ...
      <div className="grid gap-2 md:grid-cols-4">
        <Card className="p-2">
          ...
          <div className="text-2xl font-bold mb-0.5">{subscriptionsData?.total || 0}</div>
        ...
        <Card className="p-2">
          ...
          <div className="text-2xl font-bold mb-0.5">$12,450</div>
        ...
        <Card className="p-2">
          ...
          <div className="text-2xl font-bold mb-0.5">2.3%</div>
          <p className="text-xs text-muted-foreground">-0.5% from last month</p>
        </Card>
        
        <Card className="p-2">
          <div className="flex items-center justify-between mb-1">
            <h3 className="text-sm font-medium">Avg. LTV</h3>
            <Calendar className="h-4 w-4 text-muted-foreground" />
          </div>
          <div className="text-2xl font-bold mb-0.5">$890</div>
          ...
          {subscriptionsData?.pagination && (
            <div className="flex items-center justify-between mt-4">
              <div className="text-sm text-muted-foreground">
                Showing {((page - 1) * 20) + 1} to {Math.min(page * 20, subscriptionsData?.total || 0)} of {subscriptionsData?.total || 0} subscriptions
              </div>
